...
$launcherMutantSource = @{
    Text         = [IO.File]::ReadAllText((Join-Path (Get-RepoRoot) 'windows\scripts\build\Build-OnnxFromSource.ps1'))
    FunctionName = 'Disable-OrtCudaLauncherForPtx'
}
...
Describe 'Build-OnnxFromSource: the CUDA launcher and a Blackwell arch list' {
    ...
    It 'takes the launcher back for 120 or 121, and leaves any other arch list alone' {
        foreach ($c in @(
    ...
    It 'a mutant that matches 120 alone keeps the launcher for 121, so the case above bites (mutation)' {
        Invoke-WithFunctionModule @launcherMutantSource -Find "'^12[01]'" -Replace "'^120'" -Body {
            Invoke-WithEnv @{ SCCACHE_CUDA_LAUNCHER = '1' } {
                Assert-False (Disable-MutOrtCudaLauncherForPtx -Architectures '121' 6>$null) 'the mutant must miss 121'
                Assert-Equal '1' "$env:SCCACHE_CUDA_LAUNCHER" 'and leave the launcher on'
            }
        }
    }
