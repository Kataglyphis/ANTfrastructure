...
- **What stays OFF in phase 1**: OpenCV/GenAI/TVM CUDA (their cross guards remain — each needs its own arm64 CMake path) and classic TensorRT (x64-only; TensorRT-RTX is the arm64 successor and is not wired). DirectML stays ON beside CUDA, as on amd64.
- **The smoke** runs its host-toolchain §7 on this lane (nvcc is x64) with the cuDNN run-probe replaced by a link + PE-machine assert (`Assert-NativeLinkRun -CrossLinkOnly`), because the arm64 DLL cannot execute here. The payload sections still skip in-suite.
- **What no gate here proves**: that the arm64 CUDA EP *runs*. That needs an RTX Spark / N1x-class device.
