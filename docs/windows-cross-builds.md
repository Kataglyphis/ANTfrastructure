...
**Excluded — but two earlier reasons recorded here were wrong, and both are corrected as of 2026-09-19.** (1) The blanket "no Windows-on-ARM support" claim was retracted 2026-08-24, and the 421 MB cuDNN figure was never re-fetched: the cuDNN 9.25.0.15 windows-arm64 archive existed at that pin (ranged GET: HTTP 200, **~90 MB** re-measured 2026-08-28 — see backlog #122, CLOSED), `lib/arm64` inside; the current pin is `cudnn-windows-arm64-9.26.0.51_cuda13.4-archive.zip` (sha256 `657743083b72885336321403522a5af80566d8347a1b7bef5563c018d602d0d0`, 448 MB). (2) "CUDA has no `windows-arm64` redist or installer" was true on 2026-08-28 and is **false at 13.4.2**: NVIDIA ships `cuda_13.4.2_windows_arm64_network.exe` / `cuda_13.4.2_windows_arm64.exe` and 34 `windows-arm64` redist components, and its Windows-on-Arm Porting Guide documents x64→ARM64 cross-compilation (`vcvarsall x64_arm64` + `nvcc --use-local-env`, `%CUDA_PATH%\lib\arm64`). Probe-proven here 2026-09-19 (`out/probe-cuda-cross2/`): with the arm64 `cuda_cudart` archive staged into CUDA_PATH, that command produced `AA64 machine (ARM64)`. Classic TensorRT does remain genuinely x64-only — no ARM64 row in NVIDIA's support matrix — but TensorRT-RTX publishes Windows-on-Arm packages for CUDA 13.4. Wiring CUDA here is backlog **#176**, not fiction; until it lands, `Dockerfile.nvidia` is skipped and the arm64 lane always takes the CPU alias path.
