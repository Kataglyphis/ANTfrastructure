...
| `WEB_LANE_TOOLS_SOURCE=auto\|cross\|native` | Where riscv64's from-source `wasm-pack` / `flutter_rust_bridge_codegen` come from: android's cross build when usable (`auto`, default), only that (`cross`), or today's `cargo install` in the package stage (`native`). [Contract](consumer-image-contract.md#building-the-web-lane-tools-from-source). |
